...
## 데이터베이스
데이터베이스 생성
> create or replace database 데이터베이스명 옵션들...;
...
> create table test1(
  ...
  eng int,
  math int,
  constraint test1_pk/*생략가능*/ primary key(name, age)
  );
...
- 데이터가 해당 컬럼의 인덱스로 지정된 컬럼의 값이 추가/변경/삭제 될 때 인덱스 정보도 갱신한다.
...
- 단 반드시 primary key나 unique여야 한다.
```
alter table test1
...
alter table test1
  ...
  alter table test1
  add constraint unique key (no); /* 일단 no를 pk로 지정한다.*/
...
-- mysql은 오류가 나면 다음 추가시 no가 제대로 증가함.
-- 다른 DBMS(ex)오라클)에서는 입력오류가 발생하더라도 번호는 자동증가하기 때문에
