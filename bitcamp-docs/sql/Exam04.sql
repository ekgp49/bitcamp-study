...
- 하나의 테이블에 모든 데이터가 있으면 쓸 일 없는데, 여러 테이블에 데이터를 쪼개서 관리하기 위해서 사용
```
/* 게시판 테이블 */
create table test1(
  no int not null primary key auto_increment, -- artifitial key
...
```
이런 문제가 발생한 이유?
- 다른 테이블의 데이터를 참조할 때 해당 데이터의 존재유무를 검사하는 제약을 걸지 않았기 때문이다.
- 테이블에 데이터를 삭제할 때 다른 테이블이 참조하는지 여부를 검사하지 않기 때문이다.
