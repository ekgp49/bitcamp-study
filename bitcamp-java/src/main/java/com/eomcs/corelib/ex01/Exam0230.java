...
public class Exam0230 {
  public static void main(String[] args) {
    ...
    System.out.println(obj1 == obj2);// 인스턴스의 주소 비교  false
    ...
    // => 정수 리터럴을 이용하여 auto-boxing으로 생성된 객체는 constants pool에 생성된다.
    // => 같은 값을 가지는 정수 Integer 객체가 여러개 존재해야 할 필요가 없다.
    // => 그래서 가능한 이 방법을 사용해야 한다.
    // => 자바에서도 메모리 절약을 위해 이 방법을 사용하도록 유도하기 위해 wrapper 클래스의 생성자를 
    //    Deprecated로 선언하였다.
    
    Integer obj5 = Integer.valueOf(100); 
    Integer obj6 = Integer.valueOf(100); 
    System.out.println(obj5 == obj6); // true
    // Wrapper class의 valueOf()로 객체를 생성하는 경우 constants pool에 생성된다.
    // 즉 auto-boxing과 같다.
    
    // 다음과 같이 auto-boxing으로 생성된 객체와 valueOf로 생성된 객체를 비교해보면 결과는 같다.
