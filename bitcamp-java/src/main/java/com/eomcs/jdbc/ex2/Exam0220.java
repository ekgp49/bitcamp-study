...
public class Exam0220 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try (Connection con = DriverManager.getConnection( //
      ...
      //
      // 3) 바이너리 데이터 다루기
      // - Statement : 문자열로 SQL 문장을 만들기 때문에 바이너리 타입의 컬럼값을 설정할 수 없다.
      // => 바이너리 데이터를 Base64 인코딩을 통해 문자열로 변환한 후에는
      // => SQL문에 삽입하여 보낼 수 있다.
      // => 그러나 바이너리 타입의 컬럼에는 값을 설정할 수 없다.
      ...
      // 분석하지 않으므로 반복 실행할 때 Statement 보다 빠르다.
      //
    }
  }
}
