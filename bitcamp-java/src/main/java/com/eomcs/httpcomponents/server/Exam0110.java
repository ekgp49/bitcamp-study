// Apache HttpClient 라이브러리를 이용하여 파일을 리턴해주는 웹서버 만들기
...
public class Exam0110 {
  ...
  // 웹서버에서 예외가 발생했을 때 예외를 처리하는 일을 한다.
  ...
  static class HttpFileHandler implements HttpRequestHandler {
    ...
    public void handle(final ClassicHttpRequest request, // 클라이언트 요청처리 도구
      ...
      // 클라이언트가 요청한 자원의 경로
      // 예) http://localhost:9999/board/list
      // => 자원의 경로 : /board/list
      ...
      if (!file.exists()) {
        ...
        // 클라이언트가 요청한 자원(파일)이 없다면 오류메세지를 보낸다.
        ...
        // 클라이언트가 요청한 자원(파일)이 디렉토리거나 읽을 수 없다면
        // 오류메세지를 보낸다
        ...
        // 클라이언트가 요청한 자원(파일)이 있다면 읽어서 보낸다.
