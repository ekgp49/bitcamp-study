...
public class Exam0120 {
  public static void main(String[] args) {
    ...
    // 이렇게 메모리의 주소를 저장하는 변수를 레퍼런스라고 부른다.
    // 자바 primitive data type인 경우 위와 같이 그냥 변수를 선언하면 되지만 
    // class 문법으로 설계한 메모리는 이런 방식으로 만들 수 없다. 
    
    // class 문법으로 설계한 메모리(인스턴스) 준비하기.
    s = new Score(); // Score 설계도에 따라 메모리를 확보하고,
    // 그 메모리의 주소를 s에 저장한다.   
    // => 확보된 메모리를 사용하려면 주소를 보관한다.
    // => 주소를 보관하면 레퍼런스가 필요하다.
