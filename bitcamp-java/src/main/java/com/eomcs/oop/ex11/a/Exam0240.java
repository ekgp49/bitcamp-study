...
class Musics {
  class Player {
    public void play() {
      // 이 메서드가 호출되려면 Player객체가 존재햐아한다.
      // Player 객체가 존재하려면 Musics 객체가 존재해야 한다.
      // 따라서 이 메서드가 호출 될 때는 이미 Musics 객체가 존재한 상태이다.
      // 그 Musics 객체로 Player 객체를 만든 것이다.
      // 따라서 inner class 객체는 항상 자신을 만든 객체의 주소를 보관한다.
      // => 바깥클래스명.this 내장 변수에 보관한다.
      // => Player를 만든 Musics 객체를 사용하고 싶다면 바깥클래스명.this 변수를 이용하라.
  ...
  String[] list = {"aaa.mp3", "bbb.mp3", "ccc.mp3", "ddd.mp3"};
