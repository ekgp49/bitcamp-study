// local class - 인스턴스 멤버에 선언된 local class
...
public class Exam0340 {
  ...
  // 인스턴스 method의 로컬 클래스 사용법
  void im() {
    class A {
      
      // 인스턴스 메서드에서 로컬 클래스를 정의하는 경우
      // 바깥 클래스으 ㅣ인스턴스 주소를 알 수 있기 때문에
      // 컴파일러는 바깥 클래스의 인스턴스 주소를 담을 필드와 생성자를 추가한다.
      /*
       * Exam0340 this;
       * public A (Exam0340 obj) {
       *  this = obj;
       * }
       */
      void m1() {
        ...
        // [바깥클래스명].this.[인스턴스멤버]
        // Exam0340.this.iValue = 100; // OKOKOKOKOKOKOKOKOKOKOKKOKOK

        // 로컬 클래스에 같은 이름을 가진 필드가 없거나
        // 현재 메서드에 같은 이름을 가진 로컬 변수가 없을경우
    ...
    final A obj = new A();
    // 컴파일러는 위 코드를 바깥 클래스의 인스턴스 주소를 받는 생성자 호출코드로 바꾼다
