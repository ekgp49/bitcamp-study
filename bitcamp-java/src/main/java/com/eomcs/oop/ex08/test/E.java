...
public class E extends Object{
  E() { // 이렇게 파라미터가 없는 생성자를 default constructor라고 부른다.
    super(); // 모든 생성자는 첫 문장으로 수퍼 클래스의 생성자를 호출해야 한다.
  }
  
  E(int a) {
    this(); // 또는 수퍼 클래스의 생성자 대신 자기 자신의 다른 생성자를 호출할 수 있다.
  }
  
  E(String s) {
    // 수퍼클래스의 생성자나 자신의 생성자나 모두 첫 문장으로 와야하기 때문에
    // 보기와 같이 나란히 호출할 수 없다.
//    this();
//    super();
  }
